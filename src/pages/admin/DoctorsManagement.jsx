...
const DoctorsManagement = () => {
  ...
  return (
    <div className="layout-main" style={{ padding: '2rem' }}>
      ...
      {isScheduleModalOpen && selectedDoctorForSchedule && (
        <div style={{
          position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem'
        }}>
          <div className="card shadow-lg" style={{ width: '100%', maxWidth: '700px', backgroundColor: 'white', borderRadius: '20px', overflow: 'hidden' }}>
            <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f8fafc' }}>
              <div>
                <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700 }}>Manage Schedule</h3>
                <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--primary)' }}>Dr. {selectedDoctorForSchedule.doctor_name}</p>
              </div>
              <button onClick={() => setIsScheduleModalOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)' }}><FiX size={24} /></button>
            </div>
            ...
            <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>
                    {scheduleViewMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}
                  </h4>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={() => setScheduleViewMonth(new Date(scheduleViewMonth.setMonth(scheduleViewMonth.getMonth() - 1)))} style={{ padding: '0.3rem', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer' }}><FiChevronLeft /></button>
                    <button onClick={() => setScheduleViewMonth(new Date(scheduleViewMonth.setMonth(scheduleViewMonth.getMonth() + 1)))} style={{ padding: '0.3rem', borderRadius: '6px', border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer' }}><FiChevronRight /></button>
                  </div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', textAlign: 'center' }}>
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(d => <div key={d} style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', padding: '5px 0' }}>{d}</div>)}
                  
                  {Array.from({ length: getFirstDayOfMonth(scheduleViewMonth.getFullYear(), scheduleViewMonth.getMonth()) }).map((_, i) => <div key={`empty-${i}`} />)}
                  
                  {Array.from({ length: getDaysInMonth(scheduleViewMonth.getFullYear(), scheduleViewMonth.getMonth()) }).map((_, i) => {
                    const day = i + 1;
                    const dateStr = `${scheduleViewMonth.getFullYear()}-${String(scheduleViewMonth.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                    ...
                    return (
                      <button 
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          backgroundColor: isSelected ? 'var(--primary)' : (isToday ? 'rgba(82, 178, 191, 0.1)' : 'transparent'),
                          color: isSelected ? 'white' : (isToday ? 'var(--primary)' : '#334155'),
                          transition: 'all 0.2s',
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>Slots for {selectedScheduleDate}</h4>
                  <button onClick={resetToDefault} style={{ fontSize: '0.75rem', color: 'var(--primary)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>Reset to Default</button>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', maxHeight: '300px', overflowY: 'auto', paddingRight: '5px' }}>
                  {predefinedTimeSlots.map(slot => {
                    const isActive = currentDaySlots().includes(slot);
                    return (
                      <button
                        key={slot}
                        ...
                        style={{
                          ...
                          border: '1px solid',
                          borderColor: isActive ? 'var(--primary)' : '#e2e8f0',
                          backgroundColor: isActive ? 'rgba(82, 178, 191, 0.1)' : 'white',
                          color: isActive ? 'var(--primary)' : '#64748b',
                          fontSize: '0.75rem',
                      ...
                      >
                        {isActive ? <FiCheck size={12} /> : <div style={{width: 12}}></div>}
                        {slot}
                      </button>
                    );
                  })}
                ...
                <div style={{ marginTop: 'auto', paddingTop: '1rem', borderTop: '1px solid #f1f5f9' }}>
                   <p style={{ fontSize: '0.75rem', color: '#64748b', fontStyle: 'italic' }}>
                      Changes are saved automatically. Updates will sync with patient booking for this specific date.
                   </p>
                </div>
              </div>
            </div>
            
            <div style={{ padding: '1rem 2.5rem', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'flex-end', backgroundColor: '#f8fafc' }}>
              <button onClick={() => setIsScheduleModalOpen(false)} className="btn btn-primary">Done</button>
            </div>
          </div>
        </div>
      )}
    ...
    <style>
        {`
          .layout-main { min-height: 100vh; background-color: #f8fafc; }
          .page-title { color: #0f172a; }
          .btn { transition: all 0.2s; border-radius: 10px; font-weight: 600; cursor: pointer; }
          .btn-primary { background-color: var(--primary); color: white; border: none; }
          .btn-primary:hover { opacity: 0.9; transform: translateY(-1px); }
          .btn-outline { background-color: transparent; border: 1px solid #e2e8f0; color: #64748b; }
